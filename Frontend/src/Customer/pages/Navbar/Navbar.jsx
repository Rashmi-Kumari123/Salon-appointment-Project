      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        {auth.user?.id ? (
          <div className="flex gap-1 items-center">
            <h1 className="text-lg font-semibold hidden lg:block">{auth.user?.username || auth.user?.fullName}</h1>
            ...
            >
              <Avatar sx={{ bgcolor: theme.palette.primary.main }}>
                {(auth.user?.username || auth.user?.fullName) && (auth.user?.username || auth.user?.fullName)[0].toUpperCase()}
              </Avatar>
            </IconButton>
            <Menu
              id="basic-menu"
